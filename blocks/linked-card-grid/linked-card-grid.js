...
export default function decorate( block ) {
	const theme = getMetadata( 'theme' ) == 'default'? 'blue' : getMetadata( 'theme' );
	let grid =  'grid-col-12 tablet:grid-col-6 desktop:grid-col-4';
	
	const parent = block.parentElement;
	const layout = parent?.parentElement?.dataset?.layout;

	const isColored = [...block.classList].find( c => c == 'colored' || c == 'colored-filled' );
	if( isColored ) block.classList.add( theme );
